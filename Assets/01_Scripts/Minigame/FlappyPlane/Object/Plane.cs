...
{
    [SerializeField] private Animator animator = null; // 애니메이터와 리지드바디 변수 선언
    [SerializeField] private Rigidbody2D _rigidbody = null; // 플레이어의 리지드바디 (물리 엔진 적용)

    private float flapForce = 6f; // 점프 강도 (플랩)
    private float forwardSpeed = 3f; // 앞으로 나가는 속도 (수평 이동)
    public static bool isDead = false; // 플레이어가 죽었는지 확인하는 변수
    private bool isFlap = false; // 점프(플랩) 여부 확인하는 변수
    ...
    public void OnCollisionEnter2D(Collision2D collision) // 충돌 시작 시 호출
    {
        ...
        // 죽음 애니메이션 실행
        animator.SetInteger(AnimParams.IsDie, 1);
    ...
    public void OnJump(InputAction.CallbackContext context)
    {
        ...
        if (context.started) isFlap = true; // 점프 시작
    }
}
